...
async function loadBluetooth(scan = false) {
  setMessage(scan ? "Scanning Bluetooth devices..." : "Loading Bluetooth devices...");
  const devices = await request(scan ? "/api/bluetooth/scan" : "/api/bluetooth/devices", {
...
document.querySelector("#login-form").addEventListener("submit", async (event) => {
  ...
  try {
    ...
    showDashboard();
    await Promise.all([loadStatus(), loadWifi(), loadBluetooth(false)]);
  } catch (error) {
...
document.querySelector("#scan-bluetooth").addEventListener("click", () => loadBluetooth(true));
